...
def perceptron(train_ys, train_xs, dev_ys, dev_xs, args, iterations):
    ...
    for n in range(iterations):
        for i, x in enumerate(train_xs):
            if (np.dot(weights, train_xs[i]) * train_ys[i]) <= 0:
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    total_num = float(test_ys.shape[0])
    pred_y = test_ys * np.dot(test_xs, weights)
    total_correct = float(pred_y[pred_y > 0].shape[0])
    return '%.8f' % (total_correct / total_num)
...
def main():
    ...
    for i in range(50, 151):
        print(i)
        weights = perceptron(train_ys, train_xs, dev_ys, dev_xs, args, i)
        accuracy = test_accuracy(weights, dev_ys, dev_xs)
        accuracy = float(accuracy) * 100
        iterations.append(i)
        accur.append(accuracy)
    dp.draw_plot(iterations, accur)
